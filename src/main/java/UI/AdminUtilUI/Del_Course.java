 ...
 * Description:
 */
public class Del_Course extends JFrame {
    ...
    private  JRadioButton course_no;    //课程号
    private JRadioButton course_name;   //课程名
    ...
    //展示查询到的课程详细信息
    String[] columnNames={"是否选中","课程序号", "课程名", "学分", "学时", "教师编号", "上课地点"};
    Object[][] rowData = null;
    ...
    {
        String cno=del_jt.getText();
        Course course=CourseDaoImpl.query_cno_course(cno);
        if(course!=null){
            rowData=new Object[1][7];
            int i=0;
            rowData[i][0]="";
            rowData[i][1]=course.getCno();
            rowData[i][2]=course.getCname();
            rowData[i][3]=course.getCt();
            rowData[i][4]=course.getTime();
            rowData[i][5]=course.getT_no();
            rowData[i][6]=course.getLocation();
        }else {
            rowData=new Object[][]{};
        }
        update_tb();
    ...
    {
        String cno=del_jt.getText();
        courseList=CourseDaoImpl.query_cname_course(cno);
        if(courseList!=null){
            rowData=new Object[courseList.size()][7];
            int i=0;
    ...
    {
        courseList=CourseDaoImpl.query_all_course();
        if(courseList!=null){
            rowData=new Object[courseList.size()][7];
            int i=0;
            Iterator<Course> iterator=courseList.iterator();
            ...
            if(e.getSource()==query){
                ...
                if(JOptionPane.showConfirmDialog(jf,"请问您是否确认要删除选中项？","确认窗口",JOptionPane.YES_NO_OPTION)==JOptionPane.OK_OPTION)
                {
                    for(int rowindex : jTable.getSelectedRows()){
                        CourseDaoImpl.del_course((String)jTable.getValueAt(rowindex, 1));
                    }
                }
    ...
    private class MyRatioListen implements ItemListener {
        ...
        public void itemStateChanged(ItemEvent e) {
            if(e.getItem()==course_name)
            {
                if(course_name.isSelected())
                    is_no=false;
            }else if(e.getItem()==course_no)
            {
                if(course_no.isSelected())
                    is_no=true;
            }
        }
    }
    ...
    public Del_Course(){
        ...
        course_name.setBounds(370,180,80,30);
        course_no.setBounds(460,180,80,30);
        ...
        course_no.addItemListener(myRatioListen);
        course_name.addItemListener(myRatioListen);

        container.add(show_all);
        container.add(course_name);
        container.add(course_no);
    ...
    private void init(){
        top_img=new JLabel();
        Icon icon=new ImageIcon("src\\main\\resources\\del_course.jpg");
        ...
        course_name=new JRadioButton("课程名");
        course_no=new JRadioButton("课程号");
        ...
        course_no.setForeground(Color.GRAY);
        course_name.setForeground(Color.GRAY);
        buttonGroup=new ButtonGroup();
        buttonGroup.add(course_name);
        buttonGroup.add(course_no);
